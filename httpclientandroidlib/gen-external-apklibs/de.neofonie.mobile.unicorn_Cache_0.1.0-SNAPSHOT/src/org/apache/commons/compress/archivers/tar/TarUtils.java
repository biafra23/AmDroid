...
public class TarUtils {
  ...
  /**
   * Parse an octal string from a buffer. Leading spaces are ignored. The buffer must contain a trailing space
   * or NUL, and may contain an additional trailing space or NUL.
   * 
   * The input buffer is allowed to contain all NULs, in which case the method returns 0L (this allows for
   * missing fields).
   * 
   * @param buffer
  ...
  /**
   * Compute the value contained in a byte buffer. If the most significant bit of the first byte in the buffer
   * is set, this bit is ignored and the rest of the buffer is interpreted as a binary number. Otherwise, the
   * buffer is interpreted as an octal number as per the parseOctal function above.
   * 
   * @param buffer
   *          The buffer from which to parse.
   * @param offset
   ...
   * @throws IllegalArgumentException
   *           if the trailing space/NUL is missing or an invalid byte is detected in an octal number, or if a
   *           binary number would exceed the size of a signed long 64-bit integer.
  ...
  /**
   * Parse a boolean byte from a buffer. Leading spaces and NUL are ignored. The buffer may contain trailing
   * spaces or NULs.
   * 
   * @param buffer
   *          The buffer from which to parse.
  ...
  private static String exceptionMessage(byte[] buffer,
    ...
    final String s = "Invalid byte " + currentByte
                     + " at offset "
                     + (current - offset)
                     + " in '"
                     + string
  ...
  /**
   * Copy a name (StringBuffer) into a buffer. Copies characters from the name into the buffer starting at the
   * specified offset. If the buffer is longer than the name, the buffer is filled with trailing NULs. If the
   * name is longer than the buffer, the output is truncated.
   * 
   * @param name
   ...
   * 
   * Uses {@link #formatUnsignedOctalString} to format the value as an octal string with leading zeros. The
   * converted number is followed by space and NUL
   * 
   * @param value
   *          The value to write
   * @param buf
   ...
   * 
   * Uses {@link #formatUnsignedOctalString} to format the value as an octal string with leading zeros. The
   * converted number is followed by a space.
   * 
   * @param value
   ...
   * 
   * Uses {@link #formatUnsignedOctalString} to format the value as an octal string with leading zeros. The
   * converted number is followed by NUL and then space.
   * 
   * @param value
