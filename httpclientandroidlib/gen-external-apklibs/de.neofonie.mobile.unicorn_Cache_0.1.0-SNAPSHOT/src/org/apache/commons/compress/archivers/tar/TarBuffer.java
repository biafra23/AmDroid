...
/**
 * The TarBuffer class implements the tar archive concept of a buffered input stream. This concept goes back
 * to the days of blocked tape drives and special io devices. In the Java universe, the only real function
 * that this class performs is to ensure that files have the correct "block" size, or other tars will
 * complain.
 * <p>
...
class TarBuffer { // Not public, because only needed by the Tar IO streams
  ...
  /**
   * Determine if an archive record indicate End of Archive. End of archive is indicated by a record that
   * consists entirely of null bytes.
   * 
   * @param record
  ...
  /**
   * Get the current record number, within the current block, zero based. Thus, current offset =
   * (currentBlockNum * recsPerBlk) + currentRecNum.
   * 
  ...
  /**
   * Write an archive record to the archive, where the record may be inside of a larger array buffer. The
   * buffer must be "offset plus record size" long.
   * 
   * @param buf
