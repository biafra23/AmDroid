...
/**
 * Archive input streams <b>MUST</b> override the {@link #read(byte[], int, int)} - or {@link #read()} -
 * method so that reading from the stream generates EOF for the end of data in each entry as well as at the
 * end of the file proper.
 * <p>
 * The {@link #getNextEntry()} method is used to reset the input stream ready for reading the data from the
 * next entry.
 * <p>
...
public abstract class ArchiveInputStream extends InputStream {
   ...
   * 
   * MUST be overridden if the {@link #read(byte[], int, int)} method is not overridden; may be overridden
   * otherwise.
   * 
