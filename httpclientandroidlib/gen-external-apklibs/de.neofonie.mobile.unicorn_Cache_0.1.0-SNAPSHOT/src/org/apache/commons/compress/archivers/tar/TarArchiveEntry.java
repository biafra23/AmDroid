...
/**
 * This class represents an entry in a Tar archive. It consists of the entry's header, as well as the entry's
 * File. Entries can be instantiated in one of three ways, depending on how they are to be used.
 * <p>
 * TarEntries that are created from the header bytes read from an archive are instantiated with the TarEntry(
 * byte[] ) constructor. These entries will be used when extracting from or listing the contents of an
 * archive. These entries have their header filled in using the header bytes. They also set the File to null,
 * since they reference an archive entry not a file.
 * <p>
 * TarEntries that are created from Files that are to be written into an archive are instantiated with the
 * TarEntry( File ) constructor. These entries have their header filled in using the File's information. They
 * also keep a reference to the File for convenience when writing entries.
 * <p>
 * Finally, TarEntries can be constructed from nothing but a name. This allows the programmer to construct the
 * entry by hand, for instance when only an InputStream is available for writing to the archive, and the
 * header information is constructed from other information. In this case the header fields are set to
...
public class TarArchiveEntry implements TarConstants, ArchiveEntry {
  ...
  /**
   * Construct an entry for a file. File is set to file, and the header is constructed from information from
   * the file. The name is set from the normalized file path.
   * 
   * @param file
  ...
  /**
   * If this entry represents a file, and the file is a directory, return an array of TarEntries for this
   * entry's children.
   * 
   ...
   *          The tar entry header buffer to fill in.
   * @param starMode
   *          whether to use the star/GNU tar/BSD tar extension for the size field if the size is bigger than
   *          8GiB
  ...
  private static int evaluateType(byte[] header) {
    ...
    if (magic.compareTo(ByteBuffer.wrap(MAGIC_GNU.getBytes())) == 0) return FORMAT_OLDGNU;
    if (magic.compareTo(ByteBuffer.wrap(MAGIC_POSIX.getBytes())) == 0) return FORMAT_POSIX;
    return 0;
  }
}
